...
public:
    ...
    bool m_respawning;//0x00CD
    bool m_will_jack_any_player;//0x00CE
    bool m_will_jack_wanted_players; //0x00CF
    bool m_dont_drag_from_car; //0x00D0
    bool m_players_dont_drag_from_car; //0x00D1
    ...
    bool m_player_is_weird; //0x00E1
    bool m_no_critical_hits; //0x00E2
    bool disableHomingMissileLockForVehiclePedInside;//0x00E3
    bool ignoreMeleeFistWeaponDamageMult; //0x00E4
    bool lawPedsCanFleeFromNonWantedPlayer; //0x00E5
    bool hasHelmet; //0x00E6
    bool isSwitchingHelmetVisor; //0x00E7
    bool dontTakeOffHelmet; //0x00E8
    bool forceHelmetVisorSwitch; //0x00E9
    bool isPerformingVehicleMelee; //0x00EA
    bool useOverrideFootstepPtFx; //0x00EB
    bool disableVehicleCombat;//0x00EC
    bool treatFriendlyTargettingAndDamage; //0x00ED
    bool useKinematicModeWhenStationary; //0x00EE
    bool dontActivateRagdollFromExplosions; //0x00EF
    bool allowBikeAlternateAnimations; //0x00F0
    bool useLockpickVehicleEntryAnimations; //0x00F1
    bool m_PlayerPreferFrontSeat; //0x00F2
    bool ignoreInteriorCheckForSprinting; //0x00F3
    bool dontActivateRagdollFromVehicleImpact ; //0x00F4
    bool swatHeliSpawnWithinLastSpottedLocation; //0x00F5
    bool disableStartEngine; //0x00F6
    bool lawOnlyAttackIfPlayerIsWanted; //0x00F7
    bool disableHelmetArmor; //0x00F8
    bool pedIsArresting; //0x00F9
    bool isScuba; //0x00FA
    bool PRF_BlockRemotePlayerRecording ; //0x00FB
    uint32_t PRF_UseScriptedWeaponFirePosition; //0x00FC
    ...
    bool m_override_tutorial_chat; //0x0124
    bool m_override_transition_chat; //0x0125
    ...
    uint32_t m_sizeOfNetArrayData; //0x0150
    ...
    bool m_bIsPassiveMode; //0x0185
    uint8_t m_current_garage_instance_index; //0x0186
    uint8_t m_current_property_id; //0x0187
    uint8_t m_nMentalState; //0x0188
    uint8_t m_nPedDensity; //0x0189
    ...
    uint16_t m_population_control_sphere_w; //0x01AC
    uint16_t unk_01AE; //0x01AE
    uint16_t m_nCharacterRank ; //0x01B0
    bool new_01B2;
    bool new_01B3;
    bool pad_01B2; //0x01B2
    bool m_bDisableLeavePedBehind; //0x01B3
    bool m_no_collision; //0x01B4
    bool m_bInCutscene; //0x01B5
    bool m_bGhost; //0x01B6
    bool m_super_jump; //0x01B7
    bool m_EnableCrewEmblem ; //0x01B8
    bool m_ConcealedOnOwner; //0x01B9
    uint16_t m_LockOnTargetID; //0x01BA
    uint32_t m_LockOnState; //0x01BC
    float m_VehicleShareMultiplier; //0x01C0
    ...
    uint8_t m_arcadePassiveAbilityFlags; //0x01F3
    bool m_bIsShockedFromDOTEffect; //0x01F4
    bool m_bIsChokingFromDOTEffect; //0x01F5
}; //Size: 0x01F8
